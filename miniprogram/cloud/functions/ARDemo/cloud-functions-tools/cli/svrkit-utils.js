...
function main() {
  ...
  if (yargs.argv.config) {
    ...
    const outputDest =
      yargs.argv.output || path.resolve(path.dirname(configPath), 'svrkit-utils.js');
...
${staticModuleContent}`,
      ...
      pbjs.main(['-t', 'json', '-o', staticJsonFilePath, ...protos], (err, out) => {
        ...
        try {
          const protoUtils = fs.readFileSync(
            path.join(__dirname, './svrkit-utils-template.js'),
            'utf8'
          );
